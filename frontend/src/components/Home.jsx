...
function Home({ onSelectDeck }) {
    ...
    const fetchDecks = async () => {
        try {
            // Appel HTTP vers le serveur backend (Jalon 2)
            const response = await fetch('http://localhost:5000/api/decks');
            const data = await response.json();
            
            // On injecte les données de la BDD dans l'état React
            setDecks(data); 
        } catch (error) {
    ...
    return (
        <div className="container">
            ...
            {decks.length === 0 ? (
                <p>Aucun paquet trouvé. Utilisez le script de peuplement (seed) ou l'interface pour en ajouter.</p>
            ) : (
                    ...
                    {decks.map(deck => (
                        <div key={deck._id} className="deck-card">
                            ...
                            <div style={{ display: 'flex', gap: '10px', marginTop: '15px' }}>
                                {/* Événements configurés pour transmettre le choix aux Étudiants 2 et 3 */}
